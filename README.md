...
Готовый установочный образ: [`dist/Video-Editor-3.8-arm64.dmg`](dist/Video-Editor-3.8-arm64.dmg).
...
1. Скачайте и откройте [`Video-Editor-3.8-arm64.dmg`](dist/Video-Editor-3.8-arm64.dmg).
...
5. Настройте позицию, обрезку и громкость; результат можно услышать в плеере.
6. Нажмите **Старт** и выберите экспорт без дополнительного сжатия либо нужный профиль HDR/SDR.

Готовые файлы сохраняются рядом с исходником. Для редактора используется папка первого ролика.
...
Текущая версия приложения: **3.8** (build 15).
