...
Нативное приложение для сжатия, вырезания и склейки видео на Mac с процессорами Apple Silicon. Интерфейс написан на Swift/AppKit, а обработка выполняется через FFmpeg с аппаратным ускорением Apple VideoToolbox.

Готовая сборка: [`dist/Video-Editor-3.3-arm64.zip`](dist/Video-Editor-3.3-arm64.zip).
...
1. Скачайте [`Video-Editor-3.3-arm64.zip`](dist/Video-Editor-3.3-arm64.zip).
2. Распакуйте архив.
...
Отдельные терминальные утилиты в репозитории не распространяются: `video_engine` является внутренней частью `.app` и запускается только графическим интерфейсом.
...
Текущая версия приложения: **3.3** (build 6).
